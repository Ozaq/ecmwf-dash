...
- **No `aria-live` on `<main>`**: Removed the overbroad `aria-live="polite"` that announced all content on every refresh. The stale banner uses `role="alert"` for targeted announcements.
...
- **Lane-strip `max-height` clips checks silently**: `builds.css` `.lane-strip` has `max-height: 38px; overflow: hidden`, allowing ~3 rows of dots. Branches with very many checks may still have excess dots hidden — no "+N more" indicator.
- **`DeriveReviewStatus()` nil-safe**: `review.go` skips nil `*Reviewer` values in the map iteration.
- **Active nav link uses `#A66214`**: Darker than `--accent-color` for WCAG AA 4.5:1 contrast on white background. The global `--accent-color` (#C87519) is unchanged.
- **Both CI and release workflows run `govulncheck`**: Prevents shipping with known Go vulnerabilities.
