...
- **TV card flex layout**: Branch sections use `flex: 1 1 0` with `min-height: 0; overflow: hidden` so each branch gets equal card space. Headers (`card-header`, `branch-header`) use `flex-shrink: 0` to stay visible. Failure list uses `flex: 1` to fill remaining space with `align-content: flex-start` (items pack to top, not evenly distributed). Failure/running items have fixed `height: 16px` for uniform rows. Card uses `gap: 4px` (not margin-bottom) for branch spacing.
...
- **Builds use full-width grid layout**: `.build-lane` is a 2-column grid (`64px 1fr`). `.lane-strip` and `.lane-counts` both sit in column 2, stacking dots above counts. No `max-width` constraint — dots fill available width. PR `.checks-matrix` still uses `max-width: 180px`.
- **TV branch summary always rendered**: `.branch-summary` with counts is unconditional (no `HasChecks` guard) — branches without checks show all-zero dimmed counts. Keeps header layout consistent across all branches.
- **TV success uses `.all-passed` class**: When a branch has zero failures AND zero running, the template adds `.all-passed` to `.branch-section`. Counts stay inline in `.branch-summary` (same header layout as failure branches). A separate `.branch-success` div in the body holds just the affirm pill in a pastel green box (`color-mix 25% success + white`).
- **TV failure uses pastel red box**: `.failure-list` in TV mode gets a pastel red background (`color-mix 15% error + white`) with `border-radius: 6px`, mirroring the pastel green on `.branch-success`.
- **Dashboard template parses two files**: `main.go` line 69 parses both `builds_dashboard.html` and `builds.html` into the dashboard template set. Template definitions in `builds.html` are available to the dashboard. Changes to shared definitions in either file can affect the TV view.
- **TV pastel box convention**: Success and failure branches use the same box pattern — `flex: 1` container, `color-mix()` pastel background (25% success or 15% error + white), `border-radius: 6px`, `margin-top: 4px`. New TV status boxes should follow this pattern. `.branch-success` (green) and `.failure-list` (red) are the existing implementations.
